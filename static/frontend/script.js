...
function openModal(id) {
    ...
    if (id === 'clockInModal') startCamera();
...
async function apiRequest(url, options = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            // DEBUG: Log full error details to browser console
            ...
            return { 
                success: false, 
                message: data.detail || data.error || JSON.stringify(data) || `Request failed (${response.status})` 
            };
        }
...
function setupBankVerification() {
    ...
    // Auto-verify when account number is complete (10 digits)
    accountInput.addEventListener('input', debounce(async () => {
        ...
        try {
            ...
            if (res.success && res.data?.status === true && res.data?.data?.account_name) {
                ...
                const msg = res.data?.message || 'Could not verify account. Please enter name manually.';
            ...
            holderInput.value = '';
            holderInput.readOnly = false;
            showToast('Verification service unavailable. Enter name manually.', 'warning');
        } finally {
...
async function handleCreateEmployee(e) {
    ...
    const payload = {
        ...
        status: 'active'  // ADDED: Optional but good to be explicit
    };
    ...
    try {
        ...
        if (!res.success) {
            console.error('Full error:', res.data);  // DEBUG: Log exact DRF errors
            throw new Error(res.message || JSON.stringify(res.data) || 'Failed to create employee');
        }
...
async function createAccount(e) {
    ...
    const payload = {
        ...
        join_date: new Date().toISOString().split('T')[0],  // ADDED
        status: 'active'
    };
...
async function loadSackedEmployees() {
    try {
        ...
        if (!list.length) {
            tbody.innerHTML = '<tr><td colspan="6" class="text-center">No sacked employees found</td></tr>';
            return;
        }
...
async function generatePayslip() {
    ...
    try {
        ...
        const res = await apiRequest('/api/payslips/generate/', {
            method: 'POST',
...
function setupEmployeeIdGeneration() {
    ...
    if (typeSelect) typeSelect.addEventListener('change', generateId);
    if (nameInput) nameInput.addEventListener('blur', generateId); // Changed to blur for better UX
}
...
async function verifyBankAccountManual() {
    ...
    if (!bankCode) {
        showToast('Select a valid bank', 'error');
        return;
    }
    ...
    try {
        ...
        if (res.success && res.data?.valid) {
            holderInput.value = res.data.account_name;
            holderInput.style.background = '#d4edda';
